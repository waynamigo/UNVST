	...
	void initSubstrateNetwork(char *filepath){//初始化
        ...
        if(fscanf(fp,"%d%d",&nodes,&links)){
            ...
            for(int i=0;i<links;i++){
                ...
                matrix.at(templink.from).at(templink.to)=i;
                matrix[templink.to][templink.from]=i;
            }
            fclose(fp);
        }
	}
	...
	void initRequest(char *filepath){//初始化
        ...
        if(fscanf(fp,"%d%d",&nodes,&links)){
            ...
            for(int i=0;i<links;i++){
                ...
                templink.ratio=0;
                mapLinks.insert(pair<int,Link>(i,templink));
                matrix[templink.from][templink.to]=i;
                matrix[templink.to][templink.from]=i;
            }
            fclose(fp);
        }
...
int main()
{
    ...
    cout<<"物理节点顺序  ";substrateNetwork.print();cout<<endl;
    cout<<"虚拟节点顺序  ";request.print();cout<<endl;
