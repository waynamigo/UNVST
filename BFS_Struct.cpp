...
double getvnq(Request &requestnetwork,map<int,int>mappedlinks){
    double recv = mappedlinks.size();
    double requ = requestnetwork.links*1.0;
    return (1.0)*recv/requ;
}
...
int main()
{
    ...
    cout<<resLinks.size()<<endl;
    for(map<int,int>::iterator iter=resLinks.begin();iter!=resLinks.end();iter++){
        cout<<sNet[iter->first]<<'\t'<<sNet[iter->second]<<endl;
    }

    cout<<"收益开销比："<<request.getR_C(shortestpath,setMapped)<<endl;
    cout<<"网络接受率："<<getvnq(request,resLinks)<<endl;
